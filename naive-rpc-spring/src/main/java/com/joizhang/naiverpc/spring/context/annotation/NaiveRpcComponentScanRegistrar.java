...
public class NaiveRpcComponentScanRegistrar implements ImportBeanDefinitionRegistrar {
    ...
    private Set<String> getPackagesToScan(AnnotationMetadata metadata) {
        ...
        Set<String> packagesToScan =
                getPackagesToScan0(metadata, NaiveRpcComponentScan.class, "basePackages", "basePackageClasses");
        // get from @EnableDubbo, compatible with spring 3.x
        if (packagesToScan.isEmpty()) {
            packagesToScan =
                    getPackagesToScan0(metadata, EnableNaiveRpc.class, "scanBasePackages", "scanBasePackageClasses");
        }
        if (packagesToScan.isEmpty()) {
